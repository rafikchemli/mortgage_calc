...
function ContributionCard({ label, amount, percent }) {
  ...
  return (
    <div className="bg-ink-ghost rounded-lg px-4 py-3 border border-ink-ghost grid grid-cols-[56px_1fr] items-center gap-3 overflow-hidden">
      <svg width="56" height="56" viewBox="0 0 56 56">
        <circle cx="28" cy="28" r={r} fill="none" stroke="var(--s-border)" strokeWidth="4" />
        <circle
        ...
        />
        <text x="28" y="30" textAnchor="middle" fill={color} fontSize="11" fontWeight="700" style={{ fontFamily: "'Outfit', sans-serif" }}>
          {percent}%
        </text>
      </svg>
      <div className="min-w-0">
        <p className="text-[10px] text-ink-faint uppercase tracking-wider whitespace-nowrap">{label}</p>
        <p className="money text-xl mt-0.5 whitespace-nowrap"><AnimatedNumber value={amount} /><span className="text-[10px] text-ink-faint">/mo</span></p>
      </div>
    </div>
  )
}
